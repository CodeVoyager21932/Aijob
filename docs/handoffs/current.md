# 当前项目交接：G2 SME 批次 07-02 已完成，继续千家台账
...
coco 已在 P7 终局判定中选择继续扩容，并通过 ADR-0025 把企业目标从 20–30 家调整为 30–40 家；300–500 条可见岗位、SME 企业 60% 和 SME 岗位 50% 保持不变。批次 07-02 已完成：寒序科技 2 条纯实习通过，广州智跃深空、清大科越、傲冠软件、丽声助听器按硬门槛暂停；目录现为 198 总供给、134 可见、23 家企业，SME 为 7/23 家与 20/134 岗位。**当前唯一目标是按冻结排序规则继续批次 07-03，优先处理 2026-07-31 截止候选；企业达到 40 家立即停止。** 产品证据保持 `E0`，G0/G1 暂停。
...
```text
二十三个可见企业，总供给 198 条、按 ADR-0021 配额可见 134 条
...
- 当前目录：总供给 198 条、可见 134 条、23 家企业；批次 07-02 新增寒序 2 条；被压缩 64 条保留全部不可变版本并在目录页公开"X/供给 Y"。
...
- 共享适配器 `university-employment-detail-html` 已升至 0.1.2：在既有南开 correcruit / 港中深 jobview / 浙大 jyxt 契约上，继续保持职责与要求确定性分段，并新增浙大“（二）任职要求”、公开“招聘链接”、NFKC 主体比较和“每周可保证 N 个工作日”识别；company_email 继续要求企业同域与原句同时成立。
...
- 中小企业占比现状（公开分母）：有合格中小规模证据企业 7/23（红海云、先临三维、分享投资、卧安、一清、三石园、寒序），中小证据岗位 20/134；其余 `unknown` 按非中小计；距 60%/50% 目标仍显著，不降低证据标准凑比例。
- 批次 07-02：寒序 run `2ad12dcb-53f9-4748-91eb-01f8b0433cb6` 为 2 请求、2/2/0，请求间隔 2.468497 秒，同小时重放 `reused=true`；2/2 来源级自审通过，公开岗位与疑似重复为 0。
...
1. 固定 20 家队列与批次 07-02 已核验完毕；下一批继续从千家台账按 `active_explicit → active_needs_recheck`、岗位数降序、截止升序、候选 ID 升序筛选，每批最多 5 家。
...
[ ] 已确认目录为总供给 198 / 可见 134 条 local_mvp 岗位、23 家企业；通过来源 pending_review，硬冲突来源 paused
...
[ ] 已确认固定 20 家队列与批次 07-02 已核验完毕；寒序 2/2 岗位自审通过，下一批继续千家台账
