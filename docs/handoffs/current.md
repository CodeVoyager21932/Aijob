...
coco 已在 P7 终局判定中选择继续扩容，并通过 ADR-0025 把企业目标从 20–30 家调整为 30–40 家；300–500 条可见岗位、SME 企业 60% 和 SME 岗位 50% 保持不变。首轮 P1–P7 的工程、安全、50 条分类金标与真实浏览器闭环通过，但目录仍为 178 总供给、114 可见、16 家企业，SME 为 1/16 家与 1/114 岗位。**当前唯一目标是先恢复批次 03/04 已配置来源，再按固定 SME 队列执行批次 07+，企业达到 40 家立即停止。** 产品证据保持 `E0`，G0/G1 暂停。
...
1. 完成 ADR-0025、30–40 家目标与候选注册表 v2 的工程门。
2. 依次恢复全志、先临三维、分享成长、昆仑芯、鼎帷与 DTL 的真实首批和同小时幂等重放。
3. 目录硬指标仍未达：114 / 300 可见、16 / 30 家企业、SME 企业 6.25% / 60%、SME 岗位 0.88% / 50%。
4. 按固定 SME 队列执行批次 07+；每批最多 5 家，40 家仍不达标时停止并回到 P7。
