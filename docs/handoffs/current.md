...
>
> 功能实现基线：`8af6665 feat(web): enable case-derived resume editing`
>
...
- M2-1 至 M2-3 已建立真实 `/resumes` 基础资产入口、V1 只读转换、Resume V2 结构化修订编辑器，并把同一编辑器接入 Case-derived Resume；岗位简历读取创建时固定的证据修订，不会漂移到当前证据。
...
时间盒为 2–3 个有效开发日。同一时间只允许一个切片 `in_progress`；M2-0 至 M2-3 已完成，当前只执行 M2-4。
...
5. **M2-4 建议决策（当前）**：复用现有确定性建议规则与 Resume Review 真源，支持逐条接受、编辑后采用和拒绝；保存前可取消草稿，保存后恢复正文必须生成新内容修订，不篡改历史决定。首轮只用确定性模板或模拟 provider，AI 关闭仍可完成全流程。
