# 当前项目交接：ADR-0025 已批准，开始 G2 SME 扩容恢复
...
coco 已在 P7 终局判定中选择继续扩容，并通过 ADR-0025 把企业目标从 20–30 家调整为 30–40 家；300–500 条可见岗位、SME 企业 60% 和 SME 岗位 50% 保持不变。批次 07-01 已完成：固定 20 家队列全部核验，卧安机器人 4、一清创新 3、三石园科技 1 通过，目录现为 196 总供给、132 可见、22 家企业，SME 为 6/22 家与 18/132 岗位。**当前唯一目标是从千家台账按冻结排序规则继续批次 07-02，企业达到 40 家立即停止。** 产品证据保持 `E0`，G0/G1 暂停。
...
```text
二十二个可见企业，总供给 196 条、按 ADR-0021 配额可见 132 条
...
- 当前目录：总供给 196 条、可见 132 条、22 家企业；批次 07-01 新增卧安 4、一清 3、三石园 1；被压缩 64 条保留全部不可变版本并在目录页公开"X/供给 Y"。
...
- 共享适配器 `university-employment-detail-html` 已升至 0.1.1：在既有南开 correcruit / 港中深 jobview / 浙大 jyxt 契约上，新增南开“职位描述”内职责与任职要求的确定性段落边界；company_email 继续要求企业同域与原句同时成立。
...
- 中小企业占比现状（公开分母）：有合格中小规模证据企业 6/22（红海云、先临三维、分享投资、卧安、一清、三石园），中小证据岗位 18/132；其余 `unknown` 按非中小计；距 60%/50% 目标仍显著，不降低证据标准凑比例。
...
1. 固定 20 家队列已核验完毕；下一批从千家台账按 `active_explicit → active_needs_recheck`、岗位数降序、截止升序、候选 ID 升序筛选，每批最多 5 家。
2. 目录硬指标仍未达：132 / 300 可见、22 / 30 家企业、SME 企业 27.27% / 60%、SME 岗位 13.64% / 50%。
...
[ ] 已确认目录为总供给 196 / 可见 132 条 local_mvp 岗位、22 家企业；通过来源 pending_review，硬冲突来源 paused
...
[ ] 已确认固定 20 家队列已核验完毕，批次 07-01 三家、8/8 岗位自审通过，下一批转千家台账
