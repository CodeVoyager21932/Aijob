...
```text
二十三个可见企业，总供给 200 条、按 ADR-0021 配额可见 136 条
...
- 运维命令为 `source:refresh-enable/disable/status/now`，扩大多来源时可用 `source:refresh-enable --stagger-hours 24` 稳定分散当前到期任务。首次启用、扩大范围、恢复暂停与浏览器快照仍需人工明确操作；CI、测试、构建、Alpha 和 Production 均不访问真实来源。当前本机总开关为开启状态，但交接时未运行 Worker 或 `pnpm dev`。ADR-0023/0024 仍为提案。
...
- 当前目录：总供给 200 条、可见 136 条、23 家企业；批次 07-03 望尘 1 条在截止前导入，2026-08-01 转 `paused` 后不再计入当前目录；自动刷新灰度随后新增 2 条合格岗位。被压缩 64 条保留全部不可变版本并在目录页公开"X/供给 Y"。
...
1. 三来源灰度和 18 个扩展来源分散排期已完成；先观察首轮扩展运行，确认来源级失败隔离、每小时上限、熔断和目录物化在真实排期中持续稳定。
...
[ ] 已确认目录为总供给 200 / 可见 136 条 local_mvp 岗位、23 家企业；活动通过来源 pending_review，硬冲突或过期来源 paused
...
[ ] 已确认 ADR-0026 三来源灰度通过，最终矩阵为 21 个确定性来源启用、2 个浏览器来源仅提醒、4 个暂停来源关闭；本机总开关开启但 Worker 未运行
