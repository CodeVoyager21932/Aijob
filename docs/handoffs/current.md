...
时间盒为 2–3 个有效开发日。同一时间只允许一个切片 `in_progress`；M2-0 至 M2-2 已完成，当前只执行 M2-3。
...
4. **M2-3 岗位派生编辑器（当前）**：在 Case `resume` 上编辑真实 derived document，固定 Case、岗位版本、基础修订和证据修订；刷新、深链与 revision conflict 不静默覆盖。
