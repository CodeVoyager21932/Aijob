...
- Phase 1A、Phase 1B、Phase 2 领域设计、migration 023/024 的历史 Gate 记录保留；migrations 025/026/026B 已正式完成长期 owner、公共/私有 ApplicationCase 与对等 requirement context 前向修复。026B 决定“继续”；当前唯一工程切片为 `Phase 2A-027 Resume/Review Forward Repair`。
...
- 最新串行全仓测试通过 config 17、contracts 45、database 48、web 91、platform 434，共 635 项；migration 026B/Phase 2A 隔离测试 10/10，owner 删除/retention 2/2。lint 376 文件、TypeScript 和生产构建通过；`audit:ci` 按仓库策略通过，保留 1 high ignored/1 moderate 已登记 advisory。web 主包 531.59 kB，仅有既有 chunk warning；本轮没有前端交互变化。
...
| 当前切片 | `Phase 2A-027 Resume/Review Forward Repair`：正式化长期 Resume、public/private Case 派生引用、strict Content/Layout、独立 Review 聚合和 owner 删除覆盖；不注册业务 API、不调用真实 AI |
| 当前实现契约 | [PRD v0.2：本地完整 MVP](01-prd-v0.2.md)；长期 owner、公共/私有 Case 与 requirement context 已注册为 migrations 025/026/026B，Resume/Review 仍待按 ADR-0031 正式迁移 |
...
| 工程质量 | 串行全仓 635/635 通过；Phase 2A/026B 隔离 PostgreSQL 10/10、owner 删除/retention 2/2，正式迁移链已到 026B。lint 376 files、typecheck、build、audit 通过；UI 继续沿用 Phase 1B 的 1920/1280/768/320 Gate |
| AI | 单块真实 `suggestedText`、要求/证据引用、未选区块保留、编辑和真实章节 DOCX 已通过；公开环境关闭 |
| 参与者验证 | 尚未开始；G0 为 0/2，只有 coco 明确启动后才执行，G1 仍未开始 |
| 下一决定 | migration 027 完成长期 Resume/Review、strict Schema、public/private 引用、owner 删除、legacy 兼容与非破坏回退证据后，四选一：继续 Phase 2 后续领域迁移、修改、回退或停止 |
| 下一决定日期 | migration 027 证据包完成后 |
...
16. [ ] 当前实现 `Phase 2A-027 Resume/Review Forward Repair`：审查并正式化 024F 的长期 Resume、public/private Case 派生引用、strict Content/Layout 和 Review Run/Finding/Suggestion/Decision，补 owner 删除与非破坏回退；不注册 API、不调用真实 AI。
