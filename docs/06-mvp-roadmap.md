...
| 当前切片 | 审批包 01 已完成闭环：帆软、慧策、灵明光子、普渡通过低频核验、首批 5 条抽检与北森申请链接修复，并已提额到单家最多 30 条；千寻智能因飞书 ATS 动态令牌按预设暂停转人工快照候选；真实目录 146 条 |
| 当前实现契约 | [PRD v0.2：本地完整 MVP](01-prd-v0.2.md) |
| 当前产品证据 | `E0`：没有可复核目标用户行为证据；H-PROBLEM-001、H-VALUE-001 均未判定 |
| 本地岗位目录 | 146 条：普渡机器人 30、慧策 30、帆软 18、腾讯 14、字节 10、百度 10、美团 10、京东 10、南开就业网·好未来 7、灵明光子 7；单家公司不超过 30 条 |
| 来源政策 | 十个已接入来源均为 `pending_review`，只进入 `local_mvp`；公开 `/v1/jobs` 为 0 |
| 金标 | 32 个命名工程金标通过；不计为用户研究样本 |
| 工程质量 | 审批包 01 收束后全量 339 项测试、TypeScript、生产构建、256 个文件 lint 通过；八次真实探测全部 0 拒绝且重复探测幂等；真实 300–500 条目录仍未完成 |
| AI | 单块真实 `suggestedText`、要求/证据引用、未选区块保留、编辑和真实章节 DOCX 已通过；公开环境关闭 |
| 参与者验证 | 尚未开始；G0 为 0/2，只有 coco 明确启动后才执行，G1 仍未开始 |
| 下一决定 | 审批包 01 已收束（抽检通过、提额 30/家、目录 146 条）；下一决定是 coco 审批[审批包 02](evidence/ingestion/source-approval-batch-02-2026-07-26.md)候选的受控只读核验 |
| 下一决定日期 | coco 对审批包 02 作出决定时 |
...
千寻智能按审批包预设暂停：飞书 ATS 岗位接口需动态 CSRF 令牌与 Cookie 会话，不构成确定性无状态公开结构，转为人工浏览器快照候选，未逆向或复用令牌。

十个已接入来源的 `accessPolicyAccepted` 均未通过，因此只能用于本机目录。不得通过登录、验证码、代理 IP、动态签名逆向或放宽白名单满足数量。字节仅按 ADR-0016 由维护者人工读取可见 DOM，导入 CLI 本身不触网，也不进入生产 Worker；阿里仍因无法确认稳定匿名契约保持暂停，千寻智能因飞书 ATS 动态令牌保持暂停。评估和运行证据见[首批新来源记录](evidence/ingestion/new-source-batch-2026-07-20.md)与[审批包 01 执行记录](evidence/ingestion/approved-source-batch-01-2026-07-26.md)。

## 6. 下一行动

### 当前行动

1. 保留原 31 岗位正确性基线和现有 146 条目录；按 ADR-0017 扩至 300–500 条、20–30 家且中小企业为主，工程结果不计为用户价值证据。
2. 百度、京东、字节与审批包 01 四家新来源的人工抽检均已由 coco 于 2026-07-26 确认通过；帆软/慧策/灵明/普渡已提额至单家 30 条上限并完成物化。下一步收敛审批包 02（从 1000 家台账 active_explicit 中再筛最多 5 家），批准前不运行新来源真实探测。
