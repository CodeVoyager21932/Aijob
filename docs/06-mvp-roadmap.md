...
| 当前切片 | ADR-0026 三来源真实灰度已通过；21 个活动确定性来源已显式启用并分散排期，2 个浏览器来源只保留快照提醒；截至 2026-08-02 14:58，首轮扩展已完成神谷、帆软、普渡、寒序、鲸驰与慧策，继续观察其余 12 个到期确定性来源后恢复批次 07-04 |
| 当前实现契约 | [PRD v0.2：本地完整 MVP](01-prd-v0.2.md) |
| 当前产品证据 | `E0`：没有可复核目标用户行为证据；H-PROBLEM-001、H-VALUE-001 均未判定 |
| 本地岗位目录 | 有效总供给 202 条、可见 136 条（ADR-0021 配额）、23 家企业；望尘过期修订和不可变版本保留，但当前来源为 `paused`、不计入目录；合计压缩 66 条 |
| 来源政策 | 活动通过来源均为 `pending_review`、只进入 `local_mvp`；结构、主体或时效硬冲突来源保持 `paused`；数据库公共版本指针为 0，公共模式 `/v1/jobs` 为空，本机 `local_mvp` 路由返回 136 条内部预览；目录 SME 为 7/23 家、22/136 岗位 |
...
| 下一决定 | 继续观察 [ADR-0026](decisions/0026-local-automatic-source-refresh.md) 首轮扩展排期；来源级失败继续隔离，剩余到期来源运行稳定后按冻结排序规则执行批次 07-04 |
| 下一决定日期 | 首轮扩展排期完成后 |
...
|---|---|---|---|
| G2 Local Complete MVP | 30–40 家企业、300–500 条全部职能实习，中小企业不少于企业数 60% 和岗位数 50%；三种简历输入、三轴、推荐、JD 洞察、受控 AI/模板、DOCX、投递交接、五态决定和删除闭环 | 新范围终局未通过，按 ADR-0025 继续扩容 | 工程与闭环通过；当前 202 / 136 / 23，SME 7/23 与 22/136，空库最终一致性待重验，E0 不变 |
...
9. [ ] 继续观察首轮扩展排期；六个扩展来源已通过，待其余 12 个到期确定性来源完成或来源级隔离后执行批次 07-04，只推进仍活动且同时满足字段、主体/申请链、规模和访问边界的候选。
