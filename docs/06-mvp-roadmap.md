...
| 当前切片 | 批次 07-03 已完成；按 ADR-0026 实现本机自动刷新基础设施，当前进入先临三维、卧安机器人、硕方信息三来源灰度，灰度通过后才启用其余合格来源并恢复批次 07-04 |
| 当前实现契约 | [PRD v0.2：本地完整 MVP](01-prd-v0.2.md) |
| 当前产品证据 | `E0`：没有可复核目标用户行为证据；H-PROBLEM-001、H-VALUE-001 均未判定 |
| 本地岗位目录 | 总供给 198 条、可见 134 条（ADR-0021 配额）、23 家企业；望尘过期修订和不可变版本保留，但当前来源为 `paused`、不计入目录；合计压缩 64 条 |
| 来源政策 | 活动通过来源均为 `pending_review`、只进入 `local_mvp`；结构、主体或时效硬冲突来源保持 `paused`；公开 `/v1/jobs` 为 0；目录 SME 为 7/23 家、20/134 岗位 |
| 金标 | 50 条跨职能分类金标覆盖 12 个职能且 A/B 盲标 50/50 一致；40 条三轴工程金标继续通过；均不计为用户研究样本 |
| 工程质量 | 自动刷新基础设施隔离库全量测试 457/457（platform 363、web 57、config 16、contracts 15、database 6）、全仓 TypeScript、生产构建、300 文件 Biome lint 与 `git diff --check` 通过；本轮离线门未访问真实来源，P7 真实 1280px/320px 浏览器闭环基线继续有效 |
| AI | 单块真实 `suggestedText`、要求/证据引用、未选区块保留、编辑和真实章节 DOCX 已通过；公开环境关闭 |
| 参与者验证 | 尚未开始；G0 为 0/2，只有 coco 明确启动后才执行，G1 仍未开始 |
| 下一决定 | 完成 [ADR-0026](decisions/0026-local-automatic-source-refresh.md) 三来源真实灰度、重启幂等和目录核对；通过后分散启用其余合格来源，再继续批次 07-04 |
| 下一决定日期 | 三来源灰度完成后 |

工程证据见 [本机自动来源刷新基础设施验收](evidence/ingestion/source-refresh-automation-2026-08-01.md)、[G2 正确性重新验收记录](evidence/g2/correctness-reacceptance-2026-07-20.md)、[验收反馈修正记录](evidence/g2/acceptance-followup-2026-07-20.md)、[全部职能扩容离线基础验收](evidence/g2/all-function-expansion-foundation-2026-07-20.md)和[新公司官方来源首批评估与低频探测](evidence/ingestion/new-source-batch-2026-07-20.md)；旧工程基线见 [2026-07-18 工程验收记录](evidence/g2/local-complete-mvp-engineering-2026-07-18.md)。
...
|---|---|---|---|
| G2 Local Complete MVP | 30–40 家企业、300–500 条全部职能实习，中小企业不少于企业数 60% 和岗位数 50%；三种简历输入、三轴、推荐、JD 洞察、受控 AI/模板、DOCX、投递交接、五态决定和删除闭环 | 新范围终局未通过，按 ADR-0025 继续扩容 | 工程与闭环通过；当前 198 / 134 / 23，SME 7/23 与 20/134，空库最终一致性待重验，E0 不变 |
...
- [x] 批次 07-03：7 月 31 日截止的 5 家完成核验；望尘 1 条在截止前完成 1 请求、1/1/0、同小时幂等与 1/1 自审；共享高校适配器升至 `0.1.3` 并排除港中深页脚污染；截止后来源转 v3 `paused`，当前目录回到 198 / 134 / 23，SME 7/23 与 20/134。
...
8. [ ] 灰度先临三维、卧安机器人与硕方信息；验证计划运行、重启幂等、零硬冲突和正确物化后，才分散启用其余合格来源。
9. [ ] 自动刷新稳定后执行批次 07-04；只推进仍活动且同时满足字段、主体/申请链、规模和访问边界的候选。
