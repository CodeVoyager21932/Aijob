...
| 当前切片 | 批次 07-02 已完成：千家台账首组 5 家中寒序科技通过 2 条纯实习岗位，其余 4 家按规模、申请链或字段硬门槛暂停；下一批优先处理 2026-07-31 截止候选 |
| 当前实现契约 | [PRD v0.2：本地完整 MVP](01-prd-v0.2.md) |
| 当前产品证据 | `E0`：没有可复核目标用户行为证据；H-PROBLEM-001、H-VALUE-001 均未判定 |
| 本地岗位目录 | 总供给 198 条、可见 134 条（ADR-0021 配额）、23 家企业；批次 07-02 新增寒序 2 条；合计压缩 64 条 |
| 来源政策 | 通过来源均为 `pending_review`、只进入 `local_mvp`；结构或主体硬冲突来源保持 `paused`；公开 `/v1/jobs` 为 0；目录 SME 为 7/23 家、20/134 岗位 |
| 金标 | 50 条跨职能分类金标覆盖 12 个职能且 A/B 盲标 50/50 一致；40 条三轴工程金标继续通过；均不计为用户研究样本 |
| 工程质量 | 批次 07-02 隔离库全量测试 406/406（platform 313、web 57、config 16、contracts 15、database 5）、全仓 TypeScript、生产构建、280 文件 Biome lint 与 `git diff --check` 通过；P7 真实 1280px/320px 浏览器闭环基线继续有效 |
| AI | 单块真实 `suggestedText`、要求/证据引用、未选区块保留、编辑和真实章节 DOCX 已通过；公开环境关闭 |
| 参与者验证 | 尚未开始；G0 为 0/2，只有 coco 明确启动后才执行，G1 仍未开始 |
| 下一决定 | 按 [ADR-0025](decisions/0025-extend-g2-sme-company-cap-and-batch-authorization.md) 继续批次 07-03；先处理 2026-07-31 截止且已满足四项硬门槛的候选，不以次日过期页面补录 |
| 下一决定日期 | 当前批次完成后 |
...
6. [ ] 执行批次 07-03；优先处理 2026-07-31 截止候选，次日不沿用已过期活动性结论。
7. G0/G1 不启动，产品证据保持 `E0`；公开 `/v1/jobs`、公开 AI 与手机端专属实现继续关闭。R1 的 ADR-0023/0024 仍为提案。
