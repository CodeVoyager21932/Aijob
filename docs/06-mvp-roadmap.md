...
| 当前切片 | 批次 07-03 已完成：望尘科技 1 条在 2026-07-31 截止前通过，次日按截止日把来源升为 v3 `paused`、关闭探测并从当前目录下架；其余 4 家按字段、主体/申请链或规模硬门槛暂停 |
...
| 工程质量 | 批次 07-03 隔离库全量测试 412/412（platform 319、web 57、config 16、contracts 15、database 5）、全仓 TypeScript、生产构建、281 文件 Biome lint 与 `git diff --check` 通过；P7 真实 1280px/320px 浏览器闭环基线继续有效 |
| AI | 单块真实 `suggestedText`、要求/证据引用、未选区块保留、编辑和真实章节 DOCX 已通过；公开环境关闭 |
| 参与者验证 | 尚未开始；G0 为 0/2，只有 coco 明确启动后才执行，G1 仍未开始 |
| 下一决定 | 按 [ADR-0025](decisions/0025-extend-g2-sme-company-cap-and-batch-authorization.md) 继续批次 07-04；只处理 2026-08-01 仍活动且满足四项硬门槛的候选，不回访或补录 07-03 已截止页面 |
| 下一决定日期 | 当前批次完成后 |
...
7. [ ] 执行批次 07-04；只推进仍活动且同时满足字段、主体/申请链、规模和访问边界的候选。
8. G0/G1 不启动，产品证据保持 `E0`；公开 `/v1/jobs`、公开 AI 与手机端专属实现继续关闭。R1 的 ADR-0023/0024 仍为提案。
