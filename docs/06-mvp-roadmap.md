...
- M4-0 决定为 **修改后继续**：当前唯一切片切换为 `M4-1 兼容入口与写边界`。先阻断旧并行写入并保持旗标回退，再执行单项删除、真实保留模式和错误恢复；不得直接删除旧字段、历史内容或旗标回退路径。
...
| 当前唯一目标 | `M4-1 兼容入口与写边界`：V2 下保留岗位事实、外链、Case 创建及 `/resume` 解析确认，停止旧 Match/Decision/Tailoring/Recommendation/Insight 新写入；旧 Tailoring 只读，旧数据 URL 无损进入新设置，旗标关闭行为不变 |
...
| 当前下一决定 | 完成 M4-1 focused 工程与旗标回退检查后，只选择继续 M4-2A、修改、回退或停止；不得用页面隐藏冒充删除已经实现 |
| 时间盒 | M4-0 已完成；修正后 M4 剩余约 2.75–3.25 个有效开发日，M4-1 为 0.5 日 |
...
- [ ] `M4-1 兼容入口与写边界`（当前，0.5 日）：V2 下停止 JobDetail 的旧 Match/Decision/Tailoring/外链跟踪写入；Recommendation/Insight 变为零请求兼容说明；旧 Tailoring 只读；`/resume` 完成出口进入 `/resumes`；旧数据 URL 进入新设置。旗标关闭行为保持不变。
- [ ] `M4-2A 单项删除与选择性级联`（1–1.5 日）：复用现有 `deleted_at`、detach guard 和 owner epoch，为 Case、Resume、Interview、Debrief 接 owner-protected 删除；Case 删除逐项选择删除或脱离派生资产。
