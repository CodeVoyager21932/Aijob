...
| 当前切片 | 百度和京东各以 1 次官方列表请求新增 10 条；ADR-0016 允许字节人工浏览器可见 DOM 快照，首批零网络导入 10 条明确实习岗位；阿里仍暂停 |
| 当前实现契约 | [PRD v0.2：本地完整 MVP](01-prd-v0.2.md) |
| 当前产品证据 | `E0`：没有可复核目标用户行为证据；H-PROBLEM-001、H-VALUE-001 均未判定 |
| 本地岗位目录 | 61 条：腾讯 14、南开就业网·好未来 7、美团 10、百度 10、京东 10、字节 10 |
| 来源政策 | 六个已接入来源均为 `pending_review`，只进入 `local_mvp`；公开 `/v1/jobs` 为 0 |
| 金标 | 32 个命名工程金标通过；不计为用户研究样本 |
| 工程质量 | 字节人工导入后通过 300 项测试、TypeScript、生产构建、225 文件 lint、零网络导入幂等和 1000 岗位单 PostgreSQL 容量回归；真实 100–200 条目录仍未完成 |
| AI | 单块真实 `suggestedText`、要求/证据引用、未选区块保留、编辑和真实章节 DOCX 已通过；公开环境关闭 |
| 参与者验证 | 尚未开始；G0 为 0/2，只有 coco 明确启动后才执行，G1 仍未开始 |
| 下一决定 | coco 分别审阅百度、京东、字节至少 5 条人工抽检，并明确批准下一批真实低频探测公司后继续扩容 |
| 下一决定日期 | 三个增量来源人工抽检、下一批公司批准后 |
...
| G3 数据可持续 | 3 个来源通过准入并可独立失败；连续运行、新鲜度、追溯和官方链接达标 | 进行中，0/3 | 六个已接入来源技术可读，但均未获准公开；人工导入不计入持续性 |
...
2026-07-19 的旧工程门因 2026-07-20 运行审计发现硬条件漏投影、证据粒度失真、偏好误判和建议稿未真实改写而失效。随后完成 v4 活动要求集/唯一投影、简历文档修订/原子证据 v2、`decision-readiness-v2` 和逐块优化修复；目标美团句式、31 条离线目录、PDF/DOCX/文本、1612 字单块、推荐解释、DOCX XML、桌面和 320 px 均重新通过。原 31 岗位 G2 基线因此重新通过，但扩大后的 100–200 岗位范围仍处于 G2 重新验收，产品证据仍为 E0。
...
| 百度官方实习招聘 | 10 | 企业自有域名 SSR 首屏；单次 1 请求、10 条，申请链接为官方 UUID 详情；政策待审批 | 10 | 0 |
| 京东校园招聘实习生 | 10 | 企业自有域名公开实习列表接口；单次 1 请求、10 条，申请链接为官方详情；政策待审批 | 10 | 0 |
| 字节校园招聘人工快照 | 10 | 企业自有域名可见 DOM；人工逐条核对、离线零网络导入、只收明确实习；动态签名不逆向，政策待审批 | 10 | 0 |

六个已接入来源的 `accessPolicyAccepted` 均未通过，因此只能用于本机目录。不得通过登录、验证码、代理 IP、动态签名逆向或放宽白名单满足数量。字节仅按 ADR-0016 由维护者人工读取可见 DOM，导入 CLI 本身不触网，也不进入生产 Worker；阿里仍因无法确认稳定匿名契约保持暂停。评估和运行证据见[首批新来源记录](evidence/ingestion/new-source-batch-2026-07-20.md)。

## 6. 下一行动

### 当前行动

1. 保留原 31 岗位正确性基线；当前 61 条本地目录继续向 100–200 条扩容并重新验收，工程结果不计为用户价值证据。
2. 百度、京东和字节各 10 条增量已完成来源追溯、要求集、官方申请链接和幂等回归，但三个来源均尚未完成 coco 的至少 5 条人工抽检。
3. 下一步为滴滴、快手、网易形成第二个明确低频探测批次；仍需 coco 对该批公司明确批准，未批准前不访问招聘站。
4. 扩容后的 G2 通过前暂停 G0；之后再依次执行 2 人、6 人和补充样本。
5. G3 继续保持 0/3，六个已接入来源均为 `pending_review`；人工浏览器导入不计来源持续性，来源准入通过前公开目录保持为空。
