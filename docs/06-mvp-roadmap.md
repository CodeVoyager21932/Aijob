...
| 当前切片 | G2 SME 扩容恢复：先恢复批次 03/04 已配置来源，再按固定 SME 队列执行批次 07+；每批最多 5 家，40 家为停止线 |
...
| 下一决定 | 执行 [ADR-0025](decisions/0025-extend-g2-sme-company-cap-and-batch-authorization.md)：完成治理与候选注册表升级，通过隔离工程门后重放批次 03/04 |
| 下一决定日期 | 当前批次完成后 |
...
|---|---|---|---|
| G2 Local Complete MVP | 30–40 家企业、300–500 条全部职能实习，中小企业不少于企业数 60% 和岗位数 50%；三种简历输入、三轴、推荐、JD 洞察、受控 AI/模板、DOCX、投递交接、五态决定和删除闭环 | 新范围终局未通过，按 ADR-0025 继续扩容 | 工程与闭环通过；当前 178 / 114 / 16，SME 1/16 与 1/114，空库网络恢复未完成，E0 不变 |
...
2. [ ] 升级内部候选注册表并完成隔离工程门。
3. [ ] 依次恢复全志、先临三维、分享成长、昆仑芯、鼎帷与 DTL；来源失败独立暂停，同小时三个无关来源传输重置时停止全部真实操作。
4. [ ] 按固定 SME 队列执行批次 07+，每批最多 5 家、每家首批最多 5 条，逐批重物化并公开实际分母。
5. G0/G1 不启动，产品证据保持 `E0`；公开 `/v1/jobs`、公开 AI 与手机端专属实现继续关闭。R1 的 ADR-0023/0024 仍为提案。
