...
| 当前切片 | 批次 03/04 恢复已收束，开始按固定 SME 队列执行批次 07+；每批最多 5 家，40 家为停止线 |
| 当前实现契约 | [PRD v0.2：本地完整 MVP](01-prd-v0.2.md) |
| 当前产品证据 | `E0`：没有可复核目标用户行为证据；H-PROBLEM-001、H-VALUE-001 均未判定 |
| 本地岗位目录 | 总供给 188 条、可见 124 条（ADR-0021 配额）、19 家企业；恢复先临三维 8、分享投资 1、北京鼎帷 1；合计压缩 64 条 |
| 来源政策 | 通过来源均为 `pending_review`、只进入 `local_mvp`；全志、昆仑芯、DTL 已转 `paused`；公开 `/v1/jobs` 为 0；目录 SME 为 3/19 家、10/124 岗位 |
...
| 下一决定 | 执行 [ADR-0025](decisions/0025-extend-g2-sme-company-cap-and-batch-authorization.md)：按固定 SME 队列开始批次 07，每家先完成规模、活动性、字段与申请链四个硬门槛 |
| 下一决定日期 | 当前批次完成后 |
...
|---|---|---|---|
| G2 Local Complete MVP | 30–40 家企业、300–500 条全部职能实习，中小企业不少于企业数 60% 和岗位数 50%；三种简历输入、三轴、推荐、JD 洞察、受控 AI/模板、DOCX、投递交接、五态决定和删除闭环 | 新范围终局未通过，按 ADR-0025 继续扩容 | 工程与闭环通过；当前 188 / 124 / 19，SME 3/19 与 10/124，空库最终一致性待重验，E0 不变 |
