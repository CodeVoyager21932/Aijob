...
| 工程质量 | 227 项测试、全工作区类型检查、生产构建、190 文件 lint 和生产依赖审计通过 |
...
- [x] OpenAI-compatible HTTPS 适配层、环境密钥、超时、响应大小、无工具调用和 Schema 校验。
...
1. coco 只在本机 `.env` 设置 `ENABLE_AI=true`、`AI_BASE_URL`、`AI_MODEL` 和 `AI_API_KEY`；密钥不进入聊天或 Git。
2. 使用合成、去标识化、已确认证据做一次真实兼容接口冒烟，并检查无新增事实、无工具调用和模板降级。
