...
| 下一决定 | 批次 02 与千寻快照已执行完毕（[执行记录](evidence/ingestion/approved-source-batch-02-execution-2026-07-26.md)）；下一决定是 coco 逐条抽检批次 02 首批导入（硕方 5、鲸驰 1、神谷 1、红海云 1）与千寻快照 7 条，并决定鹏扶暂停后是否从备选池替换 |
| 下一决定日期 | coco 完成抽检时 |
...
| G3 数据可持续 | 3 个来源通过准入并可独立失败；连续运行、新鲜度、追溯和官方链接达标 | 进行中，0/3 | 十个已接入来源技术可读，但均未获准公开；人工导入不计入持续性 |
...
2. 审批包 02（纯 SME 版）与千寻快照批次已执行完毕；下一步等待 coco 逐条抽检批次 02 首批 8 条与千寻 7 条，决定鹏扶替换与批次 03 名单（台账建议：昆仑芯、鼎帷、Flab、分享成长、DTL量化），批准前不运行新来源真实探测。
3. 按 ADR-0019 完成 1000/1000 家企业/机构审查记录：前 300 家在主台账，301–1000 家分批保存在七个暂存表。合计 113 家有明确活动实习证据、227 家需要继续复核、362 家仅为 `discovery_only`、296 家岗位已截止、2 家命中为非岗位活动。301–1000 家中 699 家有岗位级或企业官方实习证据、1 家只有已结束的企业实习专场证据；历史与当前合并的方向发现覆盖为电子信息技术 262 家、产品/运营 200 家、两类重叠 88 家，374/700 家至少覆盖一个优先方向，但 `active_explicit` 中对应数量仅为 48 家、39 家和 18 家。123 家记录到企业官方 URL 或企业域名邮箱，其中只有 74 家尚未标为截止；最后 100 家均为历史岗位级发现，没有新增当前申请链。浙江精准学因企业页实际导向 BOSS 已从官方申请链降级。公司行业没有替代岗位方向证据。没有一家因进入台账而自动获得探测或导入批准；只有筛选通过的最多 5 家才进入 coco 审批包。
4. 最小 JD 洞察已经实现，停止继续扩功能；完成新目录、分类金标、公司均衡统计和原句追溯回归后重新判断 G2，再依次执行 2 人 G0 与 6 人 G1。
5. G3 继续保持 0/3，十个已接入来源均为 `pending_review`；人工浏览器导入不计来源持续性，来源准入通过前公开目录保持为空。
