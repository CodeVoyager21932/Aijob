...
|---|---|
| 更新日期 | 2026-07-19 |
| 当前阶段 | Build：`G2 Local Complete MVP` 已完成；G0 尚未开始 |
| 当前切片 | 冻结 G2 工程基线，等待 coco 明确启动 2 人协议校准 |
...
| 工程质量 | 237 项测试、全工作区类型检查、生产构建、197 文件 lint 和生产依赖审计通过 |
| AI | 受控适配器、模拟接口、模板降级与真实兼容接口合成数据冒烟通过；公开环境关闭 |
| 参与者验证 | 暂停；G2 完成后先做 2 人校准，再做 6 人任务和 72 小时回访 |
| 下一决定 | 冻结 G2 构建、岗位目录与术语；由 coco 决定何时开始 G0 的 2 人协议校准 |
| 下一决定日期 | 2026-07-19 |
...
|---|---|---|---|
| G2 Local Complete MVP | 3 来源 30–100 岗位、三种简历输入、三轴、推荐、受控 AI/模板、DOCX、官方链接、五态决定和删除闭环 | 已完成 | 2026-07-19 通过；只代表本地工程可用，产品证据仍为 E0 |
...
G2 已于 2026-07-19 在真实 AI 冒烟和全量回归后通过。真实微信内浏览器真机观察放入 G0 的 2 人校准，不得以桌面浏览器截图冒充用户证据；G2 完成不改变 E0。
...
1. 冻结本次 G2 构建、31 条本地岗位目录与三轴术语，不继续以工程功能代替用户证据。
2. coco 明确开始后，使用完整 MVP 做 2 人协议校准；校准样本不计入 6 人正式任务。
3. 受影响内容修改后重新校准，再做 6 人任务与 72 小时回访；G3 来源准入可独立并行。
4. 公开 AI 仍需至少 4/6 在理解正确率或决策效率上优于模板且无新增事实错误。
