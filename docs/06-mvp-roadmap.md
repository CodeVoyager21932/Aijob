...
|---|---|
| 更新日期 | 2026-07-30 |
| 当前阶段 | 原 31 岗位 G2 基线仍有效；全部职能新范围的 P1–P7 已执行，终局报告因目录、SME 与空库恢复硬指标未达而不能判定通过，G0 暂停 |
| 当前切片 | P7 终局判定：P1–P6 留档完成，真实浏览器完整闭环与工程门通过；批次 03–06 多个官方 HTTPS 来源在当前运行环境命中 `ECONNRESET` 并独立暂停；等待 coco 四选一 |
| 当前实现契约 | [PRD v0.2：本地完整 MVP](01-prd-v0.2.md) |
| 当前产品证据 | `E0`：没有可复核目标用户行为证据；H-PROBLEM-001、H-VALUE-001 均未判定 |
| 本地岗位目录 | 总供给 178 条、可见 114 条（ADR-0021 配额）、16 家企业；新增中信证券上海分公司 1 条；千寻供给 22、帆软 18、腾讯 14、普渡 30、慧策 30，合计压缩 64 条 |
| 来源政策 | 十六个已接入来源均为 `pending_review`，只进入 `local_mvp`；公开 `/v1/jobs` 为 0；目录 SME 为红海云 1/16 家、1/114 岗位，缺口按 ADR-0020/0021 公开分母 |
| 金标 | 50 条跨职能分类金标覆盖 12 个职能且 A/B 盲标 50/50 一致；40 条三轴工程金标继续通过；均不计为用户研究样本 |
| 工程质量 | P7 隔离库全量测试 391/391（platform 298、web 57、config 16、contracts 15、database 5）、全仓 TypeScript、生产构建、276 文件 Biome lint 与 `git diff --check` 通过；真实 1280px/320px 浏览器闭环完成，推荐、洞察、模板降级、DOCX、官方链接、五态决定与删除均通过 |
| AI | 单块真实 `suggestedText`、要求/证据引用、未选区块保留、编辑和真实章节 DOCX 已通过；公开环境关闭 |
| 参与者验证 | 尚未开始；G0 为 0/2，只有 coco 明确启动后才执行，G1 仍未开始 |
| 下一决定 | coco 对 [P7 终局报告](evidence/g2/g2-reacceptance-2026-07-30.md)四选一：通过 / 带公开缺口通过 / 继续扩容并批准批次 07+ / 修订目标并新增 ADR。执行方建议先定位系统性 TLS `ECONNRESET`，再选择继续扩容 |
| 下一决定日期 | 现在 |
...
|---|---|---|---|
| G2 Local Complete MVP | 20–30 家企业、300–500 条全部职能实习，中小企业不少于企业数 60% 和岗位数 50%；三种简历输入、三轴、推荐、JD 洞察、受控 AI/模板、DOCX、投递交接、五态决定和删除闭环 | 新范围终局未通过，待 coco 判定 | 工程与闭环通过；当前 178 / 114 / 16，SME 1/16 与 1/114，空库网络恢复未完成，E0 不变 |
...
| G3 数据可持续 | 3 个来源通过准入并可独立失败；连续运行、新鲜度、追溯和官方链接达标 | 进行中，0/3 | 十六个已接入来源技术可读，但均未获准公开；人工导入不计入持续性 |
...
- [x] P1–P5 执行：中信证券上海分公司新增 1 条；批次 03–06 其余来源按过期、主体/申请链冲突或 `ECONNRESET` 分别暂停，不创建批次 07。
...
十六个已接入来源的 `accessPolicyAccepted` 均未通过，因此只能用于本机目录。不得通过登录、验证码、代理 IP、动态签名逆向或放宽白名单满足数量。字节与千寻只按 ADR-0016 使用维护者人工可见 DOM 快照，导入 CLI 本身不触网，也不进入生产 Worker。评估和运行证据见首批与批次 01–06 执行记录。

## 6. 下一行动

### 当前行动

1. coco 阅读 [P7 终局报告](evidence/g2/g2-reacceptance-2026-07-30.md)并四选一：通过 / 带公开缺口通过 / 继续扩容并批准批次 07+ / 修订目标并新增 ADR。
2. 执行方建议不要直接追加名单；先定位当前运行环境对多个官方 HTTPS 站点的系统性 TLS `ECONNRESET`，否则批次 07 仍会重复暂停。
3. 在 coco 判定前，目录保持 178 总供给、114 可见、16 家企业，公开 `/v1/jobs` 保持 0；批次 07 不创建。
4. G0/G1 不启动，产品证据保持 `E0`；G3 保持 0/3，十六个来源均为 `pending_review`。
5. R1 的 ADR-0023/0024 继续只作为提案；不在终局判定前扩功能、做手机端专属实现或公开 AI。
