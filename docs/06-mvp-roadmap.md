...
| 当前切片 | 批次 07-01 已完成：固定 20 家队列核验完毕，卧安、一清、三石园通过；下一批转入千家台账排序筛选 |
| 当前实现契约 | [PRD v0.2：本地完整 MVP](01-prd-v0.2.md) |
| 当前产品证据 | `E0`：没有可复核目标用户行为证据；H-PROBLEM-001、H-VALUE-001 均未判定 |
| 本地岗位目录 | 总供给 196 条、可见 132 条（ADR-0021 配额）、22 家企业；批次 07-01 新增卧安 4、一清 3、三石园 1；合计压缩 64 条 |
| 来源政策 | 通过来源均为 `pending_review`、只进入 `local_mvp`；结构或主体硬冲突来源保持 `paused`；公开 `/v1/jobs` 为 0；目录 SME 为 6/22 家、18/132 岗位 |
| 金标 | 50 条跨职能分类金标覆盖 12 个职能且 A/B 盲标 50/50 一致；40 条三轴工程金标继续通过；均不计为用户研究样本 |
| 工程质量 | 批次 07-01 隔离库全量测试 401/401（platform 308、web 57、config 16、contracts 15、database 5）、全仓 TypeScript、生产构建、279 文件 Biome lint 与 `git diff --check` 通过；P7 真实 1280px/320px 浏览器闭环基线继续有效 |
| AI | 单块真实 `suggestedText`、要求/证据引用、未选区块保留、编辑和真实章节 DOCX 已通过；公开环境关闭 |
| 参与者验证 | 尚未开始；G0 为 0/2，只有 coco 明确启动后才执行，G1 仍未开始 |
| 下一决定 | 固定 20 家队列已穷尽；按 [ADR-0025](decisions/0025-extend-g2-sme-company-cap-and-batch-authorization.md) 从千家台账排序筛选批次 07-02，继续四项硬门槛 |
| 下一决定日期 | 当前批次完成后 |
...
|---|---|---|---|
| G2 Local Complete MVP | 30–40 家企业、300–500 条全部职能实习，中小企业不少于企业数 60% 和岗位数 50%；三种简历输入、三轴、推荐、JD 洞察、受控 AI/模板、DOCX、投递交接、五态决定和删除闭环 | 新范围终局未通过，按 ADR-0025 继续扩容 | 工程与闭环通过；当前 196 / 132 / 22，SME 6/22 与 18/132，空库最终一致性待重验，E0 不变 |
...
5. [ ] 从千家台账按冻结排序规则执行批次 07-02；每批最多 5 家、每家首批最多 5 条。
