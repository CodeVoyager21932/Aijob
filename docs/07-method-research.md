...
| [GOV.UK Responsible AI in Recruitment](https://www.gov.uk/government/publications/responsible-ai-in-recruitment-guide/responsible-ai-in-recruitment) | 强调透明、可解释、公平、问责、质疑与救济 | AI 默认关闭；启用前需要用户实验、回归评估、供应商审查和反馈升级路径 |
...
这些调研直接支持本轮从“岗位聚合与 AI 匹配”收敛到“官方岗位投递决策助手”，并要求先验证用户决策价值，再扩大采集与自动化。当前需要守住两条顺序：

- 在扩大来源采集前，先用用户研究证明跨来源资格核对和经历证据解释确实改变投递决定。
- 在开始每个功能前，建立从问题证据到规格、计划、任务、测试、发布和结果的完整链路。

因此，当前交付按相互独立、最后汇合的证据线推进：

1. 人工礼宾实验：用官方岗位清单、来源/时效、资格与经历证据四层材料验证决策增量。
2. MVP-0：人工导入 20-30 条岗位，用规则和固定模板实现已经验证的最短决策闭环，AI 默认关闭。
3. 数据切片：从 1 个审核来源验证采集契约，稳定扩至 3 个来源后才与 MVP-0 汇合。
4. Private Alpha：以匿名邀请验证 7 日真实求职周期中的高质量决定、复访和运行稳定性。
