...
5. 触网边界不变，按 `AGENTS.md` 原文：配置中已显式启用的确定性来源可由本机 `collector-worker` 定时刷新，首次启用、扩大请求范围、恢复暂停来源和浏览器快照需人工明确操作；CI、构建、Alpha、Production 不访问真实招聘站。（本条初稿写作「触网仍需人工逐批显式授权」，那是对既有规则的误述而非新决定，已按原文更正。）
...
已落地北森族并按族逐步迁移；既有完整形状（`schemaVersion: "1.0.0"`）继续可用，两种形状走同一条校验与 transform。实测 5 个租户 **790 行 → 539 行**，每新增一个租户从约 158 行降到约 87 行。不丢信息由 `fixtures/source-configs/legacy-beisen/` 冻结的迁移前原文证明：归一化后的准入判定逐字段相等。

**修正**：本 ADR 原把租户层设想为「3–5 个字段」，实测偏乐观。租户层确实只需 4 个租户参数（子域、类目、入口路径、申请路径），但**组织身份与逐企业证据文本无法压缩**——那些本来就是逐企业事实，不是重复。真正被消掉的是厂商判断的重复书写。

原文未预见的结构：`sourceType`、`provenanceLevel`、`refreshCoverage`、`absencePolicy` 与结构评分**共变**，构成两种准入画像（企业自有站点 / 已核验 ATS 租户），而不是四五个独立字段。因此厂商层提供具名画像、租户选其一，既省字段也从结构上排除不自洽组合。

Moka 族适配器暂不可写：其匿名岗位列表契约从未被观察，`source-candidates.json` 记为 `anonymous_job_contract_unverified`，仓库内无任何响应夹具。已验证的只有申请页形状。解除条件与口径修正见 [SA Track 计划](../plans/supply-admission-scaleup-track.md)。
