...
- [Career OS 前后端同步改进计划](career-os-current-delivery-plan.md)：当前唯一活动计划；UX-0、OS-1 与 OS-2 五项 Gate 均已关闭，下一候选切片是 OS-3 申请看板与 Case 命令，尚未实施并等待 coco 指令。
...
- [UX-0 页面—系统—证据追踪矩阵](career-os-ux-0-end-to-end-traceability-matrix.md)：逐用例记录规范路由、Contracts、Platform/DB、权限/并发/删除语义、Web 状态、真实测试和 `R / A / E / M / X` 处置；OS-1/OS-2 关闭注记不改写 UX-0 审计时点事实，未决行关闭前不得实现对应页面。
