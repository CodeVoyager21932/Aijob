...
| M4 旧流程收口与测试候选 | 剩余 1.25–1.75 日 | 重复入口收口、删除和异常状态完整，一岗端到端可验收 | **当前唯一目标；M4-2B 执行中** |

当前剩余工作量基线为 1.25–1.75 个有效开发日。M4-0 只读审计、M4-1 旧并行写入隔离和 M4-2A 单项删除/选择性级联均已完成；剩余工作只覆盖 ADR-0031 已批准的数据真相与错误恢复、合成一岗候选和最终 Gate。每个切片结束后重估，不以继续铺设未来后端来延长阶段。
...
4. `M4-2B`（当前，0.5–0.75 日）：数据设置展示真实保留模式、完整范围和已脱离 Case 的资产；补简历确认原子提交、会话失效恢复，清理开发阶段标签与未实现主导航。
