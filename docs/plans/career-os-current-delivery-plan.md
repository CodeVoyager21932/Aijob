...
| M4 旧流程收口与测试候选 | 1–2 日 | 重复入口收口、删除和异常状态完整，一岗端到端可验收 | **当前唯一目标** |

当前剩余工作量基线为 1–2 个有效开发日。每个里程碑结束后重估；若时间盒将被突破，优先缩小未验证功能，不以继续铺设未来后端来延长阶段。
...
状态：**当前唯一目标**。时间盒为 1–2 个有效开发日，固定串行执行：

1. `M4-0`（当前，0.25–0.5 日）：审计 `/resume`、`/recommendations`、`/insights` 与新 OS 的用户任务、读写接口、真源、重复功能、可迁移内容、只读历史、删除/异常缺口和旗标回退，形成可复核矩阵；不在审计中顺手改业务。
2. `M4-1`（0.5 日）：按矩阵完成最小兼容入口收口。可无损进入新 OS 的内容使用明确引导或兼容跳转；无法唯一关联的历史内容继续只读，不删除旧字段。
3. `M4-2`（0.5 日）：只补当前一岗闭环缺少的单项/全部删除、404/409/会话过期/API 失败恢复和删除后不复活。
4. `M4-3`（0.5 日）：用合成 Case 贯通要求、岗位简历、Review、DOCX/打印、外链、显式投递、模板面试、复盘回流和全部个人数据删除。
5. `M4-4`（0.25–0.5 日）：完成全仓与浏览器 Gate，形成独立测试候选证据包，再决定继续 Private Alpha 准备、修改、回退或停止。
