...
| M1 真实 Case 工作台 | 1–2 日 | Case/要求读取与写入真实内部状态，岗位简历读取真实修订 | **当前唯一目标** |
| M2 专业简历闭环 | 2–3 日 | 简历解析确认、结构编辑、章节调整、逐条建议、两模板和导出进入同一 OS | 未开始 |
| M3 投递与持续改进 | 2–3 日 | 手动投递记录、模板文字面试、结构化反馈和复盘回流 | 未开始 |
| M4 旧流程收口与测试候选 | 1–2 日 | 重复入口收口、删除和异常状态完整，一岗端到端可验收 | 未开始 |

当前剩余工作量基线为 6–10 个有效开发日。每个里程碑结束后重估；若时间盒将被突破，优先缩小未验证功能，不以继续铺设未来后端来延长阶段。
