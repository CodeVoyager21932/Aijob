...
设计决定为 **继续**，下一切片只实现 `ApplicationCase core contracts + migration 023`。本轮没有创建表、修改运行数据库或访问真实数据。
...
- 顶层聚合包含 `expires_at`，约束 `expires_at <= created_at + interval '30 days'`；服务写入 `min(owner.retention_expires_at, created_at + 30 days)`。子表通过聚合外键和 `ON DELETE CASCADE` 不得晚于聚合存活。
...
| `expires_at` | 不晚于 owner 到期及创建后 30 天 |
...
- 列表索引：`(owner_id, updated_at DESC, id DESC) WHERE deleted_at IS NULL`；到期索引：`(expires_at, id) WHERE deleted_at IS NULL`。
...
- V2 内容仍存 `sections jsonb`；语义区块/证据 ID 在编辑和换模板时稳定。内容修订继续禁止 UPDATE。

### 6.3 `profile.resume_layout_revisions`

列：`id`、owner、`document_id`、`layout_revision`、`base_layout_revision`、`template_key`、`section_order jsonb`、`settings jsonb`、`content_hash`、`created_at`。

- `template_key` 首轮只允许 `cn_classic_single_column/cn_compact_technical`。
- `section_order` 只含稳定 section ID；`settings` 只含布局 token，不得复制正文或证据。
...
|---|---|---:|---:|
| 新 interview feedback/turn/session | Session 软删后异步物理级联 | 1 | owner/聚合到期，最长 30 天 |
| Debrief | 聚合软删后物理删除 | 2 | 30 天 |
| Knowledge links/clips | 关联/Clip 可单独物理删除 | 3 | 30 天 |
| Case requirement links/states/questions/events | 随 Case 删除 | 4 | 30 天 |
| Resume layout/content revisions/documents | 文档软删后物理级联；V1 仍由 owner 删除 | 5 | 30 天 |
| 既有 exports/tailoring/recommendation/match/insight | 沿用当前顺序，先导出后运行 | 6 | 导出 24 小时；其余 30 天 |
| 旧 job decisions | 物理删除 | 7 | 30 天 |
| profile evidence/preferences/facts/analyses | 沿用当前依赖顺序；原文确认即删、异常最迟 24 小时 | 8 | 原文 24 小时；结构化 30 天 |
...
1. **023 `application_case_core_expand`**：创建 `application` schema、Case/事件/要求状态/证据连接/问题表，补 catalog 复合唯一约束、索引、不可变触发器和显式权限；同步 Zod/Database 类型与隔离 PostgreSQL 测试。无 API、无业务写入。
2. **024 `resume_document_v2_expand`**：创建 `resume_documents`/layout revisions，给旧 content revisions 加 nullable 归属列并放宽成 V1/V2 配对约束；旧行逐字节/逐列不变；增加转换器契约测试。
3. **025 `interview_debrief_knowledge_expand`**：新增三域表、复合 owner 外键、索引、触发器和权限。
4. **026 `career_os_owner_tasks_expand`**：扩充 task type CHECK、contracts、worker allowlist/payload schema；先只支持离线 stub，尚不接真实 AI。
5. **027 `career_os_deletion_and_compatibility_expand`**：扩展 owner 删除顺序、retention、no-store 前缀、旧决定/旧 V1 读取隔离和权限测试。
