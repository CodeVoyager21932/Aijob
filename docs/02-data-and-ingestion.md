...
采集系统把经过人工审核的企业官网和企业官网确认的官方 ATS 转换为可追溯、可复查、可回放的岗位版本。高校就业网站、政府公共就业页面、企业公众号和其他二手页面只用于发现企业及其官网方向，不得直接产生用户可见岗位。采集系统不负责推荐，不接触用户简历，不执行页面中的指令，也不接受普通用户提供的任意 URL。
...
| 值 | 含义 | MVP 处理 |
|---|---|---|
| `organization_owned` | 企业自有域名的公开招聘页面/接口 | 可作为目录真源；仍需证明具体岗位、完整 JD 和申请映射 |
| `verified_ats_tenant` | 已核验企业归属和租户路径的官方 ATS 页面 | 接入精确租户范围 |
| `university_published` | 高校就业网站公开发布，企业主体可核验 | 仅 `discovery_only`；不得生成用户可见岗位或刷新官方活动性 |
| `official_account_link` | 企业认证公众号的人工可见原文 | 仅 `discovery_only`；用于定位企业官网或官网绑定 ATS |
...
前台只展示企业官网或官方 ATS 对具体岗位的最后核验时间。高校等发现线索的读取时间不能写成岗位核验时间；来源新鲜度也不能单独证明某条岗位仍在招聘。

### 2.5 目录角色 `catalog_role`

- `canonical`：企业官网或官网确认的官方 ATS；满足岗位级资格门后可进入用户目录。
- `discovery_only`：高校、政府、公众号和其他二手公开线索；只能进入候选审计。
- `disabled`：暂停、拒绝、退役或测试之外不可运行的来源。

`catalog_role` 与访问许可、采集方式和来源类型相互独立。技术上可访问、主体可辨认或页面仍在线，都不能把 `discovery_only` 提升为 `canonical`。
