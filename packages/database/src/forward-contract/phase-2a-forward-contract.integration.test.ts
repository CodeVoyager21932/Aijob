...
describeWithDatabase("migrations 026B through 035 forward repairs and Review v2 expansion", () => {
  const ids = {
  ...
  it("migrates empty and populated 025 databases through the latest repair without expiring accounts", async () => {
    ...
    expect(migration.rows[0]?.name).toBe("035_reconciled_publication");
    expect(emptyMigration.rows[0]?.name).toBe("035_reconciled_publication");
