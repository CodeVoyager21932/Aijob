...
|---|---|
| 快照日期 | 2026-08-03 |
| 当前阶段 | R2 与 G2 P1–P7 已完成首轮；coco 已通过 ADR-0027 将当前阶段调整为 Private Alpha 规模化供给，100 家企业 / 1000 条可见岗位硬门槛和后续服务器就绪 Gate 通过前，G0/G1 暂停 |
| 当前范围 | 二十九家可见企业，有效总供给 231 条、按 ADR-0021 配额可见 149 条活动本地岗位；距离硬门槛仍缺 71 家企业和 851 条可见岗位。SME 为 7/29 家、22/149 岗位；人工来源为 2/29 家、19/149 岗。跨职能分类金标 50 条、三轴工程金标 40 条。当前目录只是运行基线，不能标记为供给 Gate 通过 |
...
| 当前实现策略 | 已实现候选注册表 v4、容量型批次规划、零网络来源族审计、配置化多租户北森适配、fail-closed 空库恢复入口与本机自动来源刷新；真实来源仍按来源独立暂停，不为数量放宽主体、申请链或字段标准 |
| 来源发现进度 | 已按 ADR-0019 完成 1000/1000 家企业/机构审查记录；34 个来源配置中 27 个活动确定性来源、2 个浏览器提醒来源、5 个暂停来源。当前审计没有 `capacity` 就绪候选；北森和第二来源族真实预检需要单独授权，不能把发现记录直接计入批次 |
| 工程切片 | 规划器目录 231/149/29，公共版本 0；按当前 7/29 家 SME 计算，`40/400` 最早需要达到 44 家、其中 22 家 SME。12 小时动态刷新已通过 110 个虚拟来源离线验证，但当前策略仍保持历史每小时 3 家限制 |
...
这条链路只证明本地工程可行，不代表来源获准公开或产品价值已验证。通过来源均为 `pending_review`，全部岗位只进入本机 `local_mvp`；数据库公共版本指针仍为 0，公共模式的 `/v1/jobs` 返回空列表。本机启用 `local_mvp` 时，同一路由会按设计返回 149 条内部预览岗位。腾讯上游的聚合占位 ID（`-2`、`-3`）会被拒绝并留下复核记录，不会伪装成真实岗位。
...
`pnpm local:bootstrap` 按迁移、来源登记、允许的本地快照导入/低频探测、目录物化和健康检查顺序恢复本地目录。它只读取 Git 已忽略的 `.data/local-bootstrap.json` 清单；清单、快照或必需来源缺失时会 fail-closed，不会用空目录冒充成功。当前独立 `aijob_alpha` 已完成 18 个迁移，但本地恢复清单仍是旧的 178/114/16 分母，尚未获准运行其中的真实探测，因此不能视为当前目录恢复通过。首次准备清单和隔离库演练方式见 [本地空库恢复手册](docs/runbooks/local-bootstrap.md)。
