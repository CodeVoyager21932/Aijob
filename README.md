...
| 当前范围 | 三来源 31 条岗位与完整本地用户闭环已形成并通过工程验收；产品证据仍为 E0 |
| 协议校准 | 暂停；完整 MVP 后再做 2 人校准，历史可核验记录仍为 0/2 |
| 正式实验 | 暂停；G2 与 G0 后再做 6 人正式任务和 72 小时回访 |
| 历史研究样本 | 5 条本地产品/运营岗位；不等于完整 MVP 目录 |
| 当前证据 | E0：尚无可复核目标用户行为证据，两个产品假设均未判定 |
| 当前实现策略 | S0–S5 本地闭环工程候选已完成；G3 三来源准入与持续性仍并行推进 |
| 工程切片 | 腾讯 14、南开就业网·好未来 7、美团 10，共 31 条 `local_mvp` 岗位；237 项测试通过 |
| AI 状态 | 受控适配器、模拟接口、模板降级与真实兼容接口合成数据冒烟通过；公开环境关闭 |
...
- 完整本地 MVP 包含 3 个官方来源和 30–100 条产品/运营实习岗位；来源通过准入与持续性 Gate 后才能进入 Private Alpha。
