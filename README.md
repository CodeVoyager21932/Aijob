...
> 2026-08-12：`M3 投递与持续改进` 已通过[完整工程与浏览器总验收](docs/evidence/product/career-os-v2/m3-workflow-acceptance-2026-08-12.md)，M4-0、[M4-1 兼容入口和写边界](docs/evidence/product/career-os-v2/m4-1-legacy-write-boundary-acceptance-2026-08-12.md)与 [M4-2A 单项删除和选择性级联](docs/evidence/product/career-os-v2/m4-2a-selective-deletion-acceptance-2026-08-12.md)也已完成。当前唯一目标是 `M4-2B 数据真相与错误恢复`。旧 G4-first 严格总计划已[废止并归档](docs/plans/career-os-v2-upgrade-plan-2026-08-04.md)，不得再提供下一任务。
...
|---|---|
| 快照日期 | 2026-08-12 |
| 当前阶段 | Career OS 2.0 `M4 旧流程收口与测试候选`；M1–M3、M4-0、M4-1 与 M4-2A 已完成，当前执行 M4-2B 数据真相与错误恢复。Private Alpha 供给与服务器 Gate 通过前，G0/G1 继续暂停 |
...
| 当前实现策略 | 保留现有 PostgreSQL、模块化单体、可信来源门和受控 AI 边界；以 Case 工作台为唯一业务真源，私有 JD 仅 owner 可见。M4-1 已停止 V2 下旧 Match/Decision/Tailoring 等并行写入；M4-2A 已接通 Case、Resume、Interview、Debrief 的单项删除和显式删除/脱离选择。当前只收口真实 owner 保留模式、数据范围、简历确认原子性、会话失效恢复与用户可见开发标签 |
...
| 工程切片 | M4-2A 全仓串行回归 Config 17、Contracts 74、Database 54、Platform 458、Web 133，共 736/736；lint 439、typecheck、build、audit 与 diff check 通过。主包 562.16 kB，较 M4-1 增长约 0.28%。当前只执行 M4-2B，不实现邮箱账号、真实 AI、真实来源或 contract migration，公共版本仍为 0 |
