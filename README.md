...
> 2026-08-06：coco 已接受 [ADR-0031](docs/decisions/0031-long-lived-career-os-architecture-realignment-2026-08-06.md)。migration 025/026 已正式注册长期 owner 与公共/私有 ApplicationCase；026 复核发现私有要求上下文尚未贯通，当前唯一切片为 `Phase 2A-026B Private Requirement Context Forward Repair`。没有真实招聘来源、真实 AI、真实邮件、服务器或真实简历进入本阶段，完整顺序见 [严格开发总计划](docs/plans/career-os-v2-upgrade-plan-2026-08-04.md)。
...
|---|---|
| 快照日期 | 2026-08-06 |
| 当前阶段 | Career OS 2.0 Phase 2A；migration 025/026 已完成长期 owner、Account/EmailIdentity 与公共/私有 ApplicationCase 的 additive 前向修复，当前补齐私有 JD 要求上下文。100/1000 供给与服务器 Gate 通过前，G0/G1 继续暂停 |
...
| 当前实现策略 | 保留现有 PostgreSQL、模块化单体、可信来源门和受控 AI 边界；以 Case 工作台为唯一业务真源，职业资产长期保留并由用户主动删除，私有 JD 仅 owner 可见；下一步先让私有要求状态/证据/问题与公共 Case 具备对等契约，再注册 Resume 前向迁移 |
...
| 工程切片 | 迁移 019–026 已落实统一岗位资格、岗位级新鲜度、运行角色、ApplicationCase/Resume V2 additive 基础、长期 owner 与公共/私有 Case；Alpha 邀请门同时保护前端与后端 API，PDF/DOCX 在受限子进程解析，100 家/1000 岗候选冻结链已通过 PostgreSQL 集成验证；公共版本仍为 0 |
