...
| 当前范围 | 二十三家可见企业，有效总供给 202 条、按 ADR-0021 配额可见 136 条本地岗位；SME 为 7/23 家、22/136 岗位；跨职能分类金标 50 条、三轴工程金标 40 条。企业数、可见岗位数与 SME 占比仍未达到 G2 硬指标，不能标记为通过 |
...
| 当前实现策略 | 已实现公司规模证据、公众号/企业邮箱零网络导入边界、确定性 JD 洞察、跨职能分类金标、fail-closed 空库恢复入口与本机自动来源刷新；真实来源仍按来源独立暂停，不为数量放宽主体、申请链或字段标准 |
| 来源发现进度 | 已按 ADR-0019 完成 1000/1000 家企业/机构审查记录；依次执行 P1、批次 03–06、ADR-0025 恢复和 SME 批次 07-01/07-02/07-03。ADR-0026 三来源灰度通过后，21 个活动确定性来源已显式启用并分散排期，2 个浏览器来源仅生成提醒，4 个暂停来源保持关闭；截至 2026-08-02 14:58，首轮扩展已完成神谷、帆软、普渡、寒序、鲸驰与慧策，仍有 12 个确定性来源到期等待 |
| 工程切片 | 可见目录 136 条（配额压缩 66 条，公共版本 0）；首轮 6 家计划刷新合计 9 请求、76/76/0，全部自动接受且无熔断或自动暂停，普渡新增 2 条供给；P7 浏览器闭环与工程门结果以路线面板最新记录为准 |
...
这条链路只证明本地工程可行，不代表来源获准公开或产品价值已验证。通过来源均为 `pending_review`，全部岗位只进入本机 `local_mvp`；数据库公共版本指针仍为 0，公共模式的 `/v1/jobs` 返回空列表。本机启用 `local_mvp` 时，同一路由会按设计返回 136 条内部预览岗位。腾讯上游的聚合占位 ID（`-2`、`-3`）会被拒绝并留下复核记录，不会伪装成真实岗位。
