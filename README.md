...
> 2026-08-06：coco 已接受 [ADR-0031](docs/decisions/0031-long-lived-career-os-architecture-realignment-2026-08-06.md)。migrations 025/026/026B 已正式注册长期 owner、公共/私有 ApplicationCase 与对等 requirement context；当前唯一切片为 `Phase 2A-027 Resume/Review Forward Repair`。没有真实招聘来源、真实 AI、真实邮件、服务器或真实简历进入本阶段，完整顺序见 [严格开发总计划](docs/plans/career-os-v2-upgrade-plan-2026-08-04.md)。
...
|---|---|
| 快照日期 | 2026-08-06 |
| 当前阶段 | Career OS 2.0 Phase 2A；migrations 025/026/026B 已完成长期 owner、Account/EmailIdentity、公共/私有 ApplicationCase 与 requirement context 的 additive 前向修复，当前正式化长期 Resume/Review。100/1000 供给与服务器 Gate 通过前，G0/G1 继续暂停 |
...
| 当前实现策略 | 保留现有 PostgreSQL、模块化单体、可信来源门和受控 AI 边界；以 Case 工作台为唯一业务真源，职业资产长期保留并由用户主动删除，私有 JD 仅 owner 可见；下一步把 024F 的长期 Resume、严格 Content/Layout 与独立 Review 聚合审查后注册为 migration 027，并补 owner 删除覆盖 |
...
| 工程切片 | 迁移 019–026B 已落实统一岗位资格、岗位级新鲜度、运行角色、ApplicationCase/Resume V2 additive 基础、长期 owner、公共/私有 Case 与 requirement context；Alpha 邀请门同时保护前端与后端 API，PDF/DOCX 在受限子进程解析，100 家/1000 岗候选冻结链已通过 PostgreSQL 集成验证；公共版本仍为 0 |
