...
> 2026-08-06：coco 已接受 [ADR-0031](docs/decisions/0031-long-lived-career-os-architecture-realignment-2026-08-06.md)。migrations 025/026/026B/027 已正式注册长期 owner、公共/私有 ApplicationCase、对等 requirement context 与长期 Resume/Review；当前唯一切片为 `Phase 2A-028 Interview/Debrief/Knowledge Expand`。没有真实招聘来源、真实 AI、真实邮件、服务器或真实简历进入本阶段，完整顺序见 [严格开发总计划](docs/plans/career-os-v2-upgrade-plan-2026-08-04.md)。
...
|---|---|
| 快照日期 | 2026-08-06 |
| 当前阶段 | Career OS 2.0 Phase 2A；migrations 025/026/026B/027 已完成长期 owner、Account/EmailIdentity、公共/私有 ApplicationCase、requirement context 与 Resume/Review 的 additive 前向修复，当前扩展 Interview/Debrief/Knowledge 领域。100/1000 供给与服务器 Gate 通过前，G0/G1 继续暂停 |
...
| 当前实现策略 | 保留现有 PostgreSQL、模块化单体、可信来源门和受控 AI 边界；以 Case 工作台为唯一业务真源，职业资产长期保留并由用户主动删除，私有 JD 仅 owner 可见；Resume 正文、布局与 Review 已分离，下一步以 migration 028 补齐 Interview/Debrief/Knowledge 的长期领域契约 |
...
| 工程切片 | 迁移 019–027 已落实统一岗位资格、岗位级新鲜度、运行角色、长期 owner、公共/私有 Case 与 requirement context、长期 Resume、strict Content/Layout 和独立 Review 聚合；Alpha 邀请门同时保护前端与后端 API，PDF/DOCX 在受限子进程解析，100 家/1000 岗候选冻结链已通过 PostgreSQL 集成验证；公共版本仍为 0 |
| AI 状态 | Review 建议与 Resume 正文已分离并保持证据约束；公开环境继续关闭，Phase 2A-028 不调用真实 AI |
