...
| 当前范围 | 二十三家可见企业，总供给 200 条、按 ADR-0021 配额可见 136 条本地岗位；SME 为 7/23 家、22/136 岗位；跨职能分类金标 50 条、三轴工程金标 40 条。企业数、可见岗位数与 SME 占比仍未达到 G2 硬指标，不能标记为通过 |
...
| 来源发现进度 | 已按 ADR-0019 完成 1000/1000 家企业/机构审查记录；依次执行 P1、批次 03–06、ADR-0025 恢复和 SME 批次 07-01/07-02/07-03。ADR-0026 三来源灰度通过后，21 个活动确定性来源已显式启用并分散排期，2 个浏览器来源仅生成提醒，4 个暂停来源保持关闭 |
| 工程切片 | 可见目录 136 条（配额压缩 64 条，公开目录 0）；自动刷新灰度为卧安 5/5/0、先临 9/9/0、硕方 6/6/0，同窗口重放均不触网，目录新增 2 条合格岗位；P7 浏览器闭环与工程门结果以路线面板最新记录为准 |
