...
| 当前实现策略 | 保留现有 PostgreSQL、模块化单体、可信来源门和受控 AI 边界；以 Case 工作台为唯一业务真源，职业资产长期保留并由用户主动删除，私有 JD 仅 owner 可见；既有解析、事实确认、tailoring 和 DOCX 已接入统一的基础简历与岗位派生简历闭环，当前只运行 M2 总 Gate |
...
| 工程切片 | M2-0 至 M2-5 已完成基础资产、结构编辑、派生编辑、可追溯建议、两模板、A4 预览、隔离打印与精确修订 DOCX；当前只执行 M2-6 全仓和浏览器 Gate。M1 隔离 PostgreSQL 全仓 676/676 基线继续有效；公共版本仍为 0 |
