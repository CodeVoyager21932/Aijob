...
| 当前范围 | 十六个已接入来源，总供给 178 条、按 ADR-0021 配额可见 114 条本地岗位；跨职能分类金标 50 条、三轴工程金标 40 条。当前企业数、可见岗位数与 SME 占比均未达到 G2 硬指标，不能标记为通过 |
...
| 来源发现进度 | 已按 ADR-0019 完成 1000/1000 家企业/机构审查记录，并依次执行 P1、批次 04、03、05、供给检查点与批次 06。中信证券上海分公司新增 1 条；其余候选因截止、主体/邮箱边界或当前环境 TLS 连接重置按来源暂停，不进入批次 07 |
| 工程切片 | 可见目录 114 条（配额压缩 64 条，公开目录 0）：中信证券上海分公司 1、京东/帆软/腾讯/百度/慧策/字节/美团/普渡/千寻智能各 10、灵明光子/好未来各 7、硕方信息 6、神谷文化/红海云/鲸驰寰宇各 1；P7 浏览器闭环与工程门结果以路线面板最新记录为准 |
...
这条链路只证明本地工程可行，不代表来源获准公开或产品价值已验证。十六个已接入来源均为 `pending_review`，全部岗位只进入本机 `local_mvp`；公开 `/v1/jobs` 仍返回空列表。腾讯上游的聚合占位 ID（`-2`、`-3`）会被拒绝并留下复核记录，不会伪装成真实岗位。
