...
| 当前范围 | 六个已接入来源 61 条本地岗位：原 31 条正确性基线加百度、京东、字节各 10 条受控增量；产品证据仍为 E0 |
...
| 当前实现策略 | 百度、京东标准来源试点和字节人工浏览器辅助导入已完成；达到 100–200 条前不恢复 G2 |
| 工程切片 | 本地目录为腾讯 14、南开就业网·好未来 7、美团 10、百度 10、京东 10、字节 10；通过 300 项测试、幂等导入、1000 岗位容量回归和生产构建 |
...
这条链路只证明本地工程可行，不代表来源获准公开或产品价值已验证。六个已接入来源均为 `pending_review`，61 条岗位只进入本机 `local_mvp`；公开 `/v1/jobs` 仍返回空列表。腾讯上游的聚合占位 ID（`-2`、`-3`）会被拒绝并留下复核记录，不会伪装成真实岗位。
...
`browser_required` 来源不进入 `source:probe`。coco 明确批准的人工浏览器快照只能保存在 Git 忽略的 `.data/browser-imports/`，再用 `pnpm source:import-browser-snapshot <source-key> --file <path>` 离线导入；该命令没有招聘站网络请求能力。字节首批只接受官方页面明确标记为“实习”的岗位，正式校招全职和社会招聘会被整批拒绝。
